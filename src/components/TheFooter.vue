...
<footer class="relative bg-fixed bg-no-repeat bg-cover min-h-[700px] md:min-h-[500px] bg-center flex justify-center items-center md:bg-[url('../assets/footer_background2.jpeg')] bg-[url('../assets/footer_background_mobile.jpeg')]">
  <div class="w-full min-h-[700px] md:min-h-[500px] bg-slate-50 opacity-80 absolute bottom-0 left-0"></div>
  <div class="w-full min-h-[700px] md:min-h-[500px] absolute bottom-0 left-0">
    <div class="flex md:flex-row flex-col md:px-20 md:py-12 xl:px-80 xl:py-16 px-5 py-10 justify-center">
      <div class="basis-1/3 gap-2 flex flex-col justify-start my-4">
        ...
        <div class="flex flex-row gap-2 items-center md:mt-8 justify-center md:justify-start">
          <img class="w-5 h-5" src="../assets/telephone.png" />
          <a href="tel:+4366488716789">+43 (0) 664-8871 6789</a>
        </div>
        <div class="flex flex-row gap-2 items-center justify-center md:justify-start">
          <img class="w-5 h-5" src="../assets/email.png" />
          <button @click.prevent="modalShow=true">info@munda.at</button>
        </div>
      </div>
      <div class="basis-1/3 gap-2 flex flex-col justify-start">
        <div class="flex flex-row gap-2 items-center my-4 justify-center md:justify-start">
          <img class="w-5 h-5" src="../assets/clock.png" />
          <p class="font-bold text-black text-xl"> Öffnungszeiten</p>
        </div>
        <div class="flex flex-row gap-2 items-center justify-center md:justify-start">
          <p class="basis-1/3 md:basis-1/4 font-bold">Mo. -Fr</p>
          <p class=""> 09.00 - 19.00 Uhr</p>
        </div>
        <div class="flex flex-row gap-2 items-center justify-center md:justify-start">
          <p class="basis-1/3 md:basis-1/4 font-bold">Sa.</p>
          <p> 10.00 - 15.00 Uhr</p>
        </div>
        <div class="flex flex-row gap-2 items-center my-4 justify-center md:justify-start">
        ...
        <div class="grid grid-cols-4 gap-y-2 justify-items-center md:justify-items-start text-center md:text-left">
          <router-link v-for="item in navigation"
            ...
            class="col-span-2 transition duration-300 ease-in-out hover:scale-110 hover:translate-x-3"
          >
            {{ item.name }}
          </router-link>
        </div>
      </div>
      <div class="basis-1/3 flex flex-col justify-start my-4">
        <div class="flex flex-row gap-2 items-center mb-4 justify-center md:justify-start">
          <img class="w-5 h-5" src="../assets/review.png" />
          <p class="font-bold text-black text-xl"> Bewertungen</p>
        </div>
      </div>
    </div>
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
      <span class="text-sm text-gray-500 sm:text-center dark:text-gray-400">© 2022 <router-link :to="{ name: 'home' }" class="hover:underline">Munda™</router-link>. All Rights Reserved.
      </span>
...
export default {
  ...
  methods: {
    ...
    navigationDivider(divNum, partNum) {
      // const n = this.navigation.length
      const n = this.navigation.length % divNum == 0 ? this.navigation.length/divNum : (this.navigation.length+1)/divNum;
      // return partNum == 1 ? this.navigation.slice(0, n) : this.navigation.slice(n);
      const outPut = partNum == 1 ? this.navigation.slice(0, n) : this.navigation.slice(n);
      return outPut;
    },
    closeModal() {
